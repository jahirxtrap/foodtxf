        ...
        if (target != null && !target.isBaby()) {
            if (target instanceof Cow || target instanceof Goat) {
                if (mainHandIst.is(Items.COCOA_BEANS) || offHandIst.is(Items.COCOA_BEANS)) {
                    if (type == 0) result = new ItemStack(ModContent.CHOCOMILK_THERMOS.get());
                    else if (type == 1) result = new ItemStack(ModContent.GLASS_OF_CHOCOMILK.get());
                    addition.put(mainHandIst.is(Items.COCOA_BEANS) ? mainHandIst : offHandIst, type == 0 ? 2 : type == 1 ? 1 : 0);
                } else {
                    if (type == 0) result = new ItemStack(ModContent.MILK_THERMOS.get());
                    else if (type == 1) result = new ItemStack(ModContent.GLASS_OF_MILK.get());
                }
            } else return false;
            ...
            if (block == Blocks.WATER) {
                if (type == 0) result = new ItemStack(ModContent.WATER_THERMOS.get());
                else if (type == 1) result = new ItemStack(ModContent.GLASS_OF_WATER.get());
                sound = SoundEvents.BUCKET_FILL;
            } else if (block == Blocks.LAVA) {
                if (type == 0) result = new ItemStack(ModContent.LAVA_THERMOS.get());
                else if (type == 1) result = new ItemStack(ModContent.GLASS_OF_LAVA.get());
